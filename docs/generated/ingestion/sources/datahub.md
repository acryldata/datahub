...
Datahub is a DataHub utility or metadata-focused integration. Learn more in the [official Datahub documentation](https://datahub.com/).

The DataHub integration for Datahub covers metadata entities and operational objects relevant to this connector. It also captures stateful deletion detection.
...
| <div className="path-line"><span className="path-main">exclude_aspects</span></div> <div className="type-name-line"><span className="type-name">array</span></div> | Aspect names to exclude from entities that are being ingested. Note: This only makes sense for entities you want to ingest but without certain aspects. To completely exclude entity types, use 'urn_pattern.deny' instead. Warning: Excluding key aspects while keeping others can create invalid entities. <div className="default-line default-line-with-docs">Default: <span className="default-value">&#91;&#x27;datahubIngestionRunSummary&#x27;, &#x27;datahubIngestionCh...</span></div> |
...
- Enable [async ingestion](/docs/deploy/environment-vars#ingestion)
- Use standalone consumers ([mae-consumer](/docs/metadata-jobs/mae-consumer-job) and [mce-consumer](/docs/metadata-jobs/mce-consumer-job))
