    ...
    def add_profile_metadata(self, inspector: Inspector) -> None:
        ...
        with inspector.engine.connect() as conn:
            for row in conn.execute(
                "SELECT * FROM information_schema.mv_connection_attributes"
            ):
                with open("/home/amakarovych-ua/Projects/datahub/debug.txt", "a") as f:
                    f.write(
                        str(row.ATTRIBUTE_NAME) + " " + str(row.ATTRIBUTE_VALUE) + "\n"
                    )
